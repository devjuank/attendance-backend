...
func (r *QRCodeRepositoryImpl) GetActive() (*models.QRCode, error) {
	var qr models.QRCode
	err := r.db.Where("is_active = ? AND expires_at > ?", true, time.Now()).
...
func (r *QRCodeRepositoryImpl) GetByToken(token string) (*models.QRCode, error) {
	var qr models.QRCode
	err := r.db.Where("token = ?", token).First(&qr).Error
	if err != nil {
		return nil, err
	}
	return &qr, nil
...
func (r *QRCodeRepositoryImpl) DeactivateAll() error {
